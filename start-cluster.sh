...
chmod 600 ~/Documents/kube-cluster-vprofile/.vagrant/machines/*/virtualbox/private_key
...
if ! ANSIBLE_HOST_KEY_CHECKING=False ansible -i k8s-cluster-setup/ansible-ubuntu/inventory.ini all -m ping; then
...
if ! ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i k8s-cluster-setup/ansible-ubuntu/inventory.ini k8s-cluster-setup/ansible-ubuntu/playbook.yml; then
